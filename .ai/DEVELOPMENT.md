...
**Interact through public API.** Tests should drive the component through its public interface. The only exception is `#[cfg(test)]` accessors for observing internal state (e.g., checking that tracking maps are cleaned up). These accessors should be read-only. Avoid `#[cfg(test)]` helpers that inject internal state — if unavoidable, acknowledge the tradeoff with a comment.

**Match on specific error variants, not `is_err()`.**
```rust
// BAD — passes for any error, may be testing the wrong code path
assert!(resp.is_err());

// GOOD — verifies the exact scenario you intended
assert!(matches!(resp, Err(SpecificError::Variant { .. })));
```

**Targeted assertions over aggregate counts.** Check the specific entry changed, not a count delta that breaks if unrelated batching logic changes.
```rust
// BAD — fragile if batch count changes
assert_eq!(count_after, count_before - 1);

// GOOD — checks exactly what matters
assert!(!ctx.has_entry(id));
```

**Name load-bearing side effects.** If a variable's construction has a critical side effect (e.g., populating the chain), name it to prevent "dead code" cleanup. `_block_with_blobs` not `_block`.

**GIVEN/WHEN/THEN for complex tests.** When setup, action, and assertion aren't obvious at a glance, use section comments:
```rust
// GIVEN: a custody backfill entry expecting column 0 at epoch 0
...
// WHEN: a response arrives with no columns
...
