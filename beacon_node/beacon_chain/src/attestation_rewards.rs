...
pub fn compute_attestation_rewards<T: BeaconChainTypes>(
    ...
    if state.fork_name_unchecked().altair_enabled() {
        compute_attestation_rewards_altair::<T>(state, validators, spec)
    } else {
...
fn compute_attestation_rewards_base<T: BeaconChainTypes>(
    ...
    process_justification_and_finalization_base(&state, &validator_statuses.total_balances, spec)?
        .apply_changes_to_state(&mut state);

    let ideal_rewards =
        compute_ideal_rewards_base::<T>(&state, &validator_statuses.total_balances, spec)?;
    ...
    let indices_to_attestation_delta = if validators.is_empty() {
        ...
        let validator_indices = validators_ids_to_indices::<T>(&mut state, validators)?;
...
fn compute_attestation_rewards_altair<T: BeaconChainTypes>(
    ...
    for flag_index in 0..PARTICIPATION_FLAG_WEIGHTS.len() {
        let weight =
            get_flag_weight(flag_index).map_err(|_| BeaconChainError::AttestationRewardsError)?;
        ...
        for effective_balance_eth in
            1..=max_effective_balance_increment_steps(previous_epoch, spec)?
        {
    ...
    let validators = if validators.is_empty() {
        all_eligible_validator_indices::<T>(&state, previous_epoch)?
    } else {
        validators_ids_to_indices::<T>(&mut state, validators)?
    };
            ...
            for flag_index in 0..PARTICIPATION_FLAG_WEIGHTS.len() {
                ...
                if voted_correctly {
                    ...
                    let penalty_numerator =
                        effective_balance.safe_mul(state.get_inactivity_score(validator_index)?)?;
...
fn all_eligible_validator_indices<T: BeaconChainTypes>(
...
fn validators_ids_to_indices<T: BeaconChainTypes>(
...
fn compute_ideal_rewards_base<T: BeaconChainTypes>(
    state: &BeaconState<T::EthSpec>,
    ...
    for effective_balance_step in 1..=max_effective_balance_increment_steps(previous_epoch, spec)? {
        let effective_balance =
