...
/// components that implement beacon-chain logic. Previously named `BeaconSystem`;
/// the alias above keeps external crates compiling while the rename propagates.
pub struct BeaconSystem<T: BeaconChainTypes> {
    pub spec: Arc<ChainSpec>,
    /// Configuration for `BeaconSystem` runtime behaviour.
    ...
    /// The genesis time of this `BeaconSystem` (seconds since UNIX epoch).
    pub genesis_time: u64,
    /// A handler for events generated by the beacon chain. This is only initialized when the
    /// HTTP server is enabled.
    ///
    /// Also Arc-cloned onto `BlockImporter<T>`. Kept here because 25+ call sites in `http_api`,
    /// `network`, `canonical_head`, `block_verification`, and `execution_methods` access it
    /// directly; migrating them all to `block_importer.event_handler` would be too invasive.
    ...
    ///
    /// Also Arc-cloned onto `BlockImporter<T>`. Kept here because 25+ call sites in `http_api`,
    /// `network`, `state_advance_timer`, `metrics`, `attestation_simulator`, and
    /// `block_verification` access it directly.
...
/// Returns the current heads of the `BeaconSystem`. For the canonical head, see
...
// ---------------------------------------------------------------------------
// Free functions: methods extracted from `impl BeaconSystem<T>`
// ---------------------------------------------------------------------------
...
pub async fn per_slot_task<T: BeaconChainTypes>(chain: &Arc<BeaconSystem<T>>) {
...
///
/// This is a free function extracted from `BeaconSystem` to avoid coupling to `self`. It loads the
