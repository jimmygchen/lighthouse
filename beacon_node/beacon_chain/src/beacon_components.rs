...
/// components that implement beacon-chain logic. Previously named `BeaconComponents`;
...
pub struct BeaconComponents<T: BeaconChainTypes> {
    ...
    /// Configuration for `BeaconComponents` runtime behaviour.
    ...
    /// The genesis time of this `BeaconComponents` (seconds since UNIX epoch).
...
/// Returns the current heads of the `BeaconComponents`. For the canonical head, see
...
///
/// This is a free function extracted from `BeaconComponents` to avoid coupling to `self`. It loads the
