...
use crate::data_availability_checker::{
    Availability, AvailabilityCheckError, AvailableBlock, DataColumnReconstructionResult,
};
...
use store::StoreOp;
use task_executor::{RayonPoolType, ShutdownReason};
...
pub(crate) struct BlockImportContext<'a, T: BeaconChainTypes> {
    ...
    pub validator_monitor: &'a RwLock<ValidatorMonitor<T::EthSpec>>,
    pub slasher: Option<&'a Arc<Slasher<T::EthSpec>>>,
    pub event_handler: Option<&'a ServerSentEventHandler<T::EthSpec>>,
    pub data_availability_checker: &'a Arc<DataAvailabilityChecker<T>>,
    pub observed_slashable: &'a RwLock<ObservedSlashable<T::EthSpec>>,
    pub spec: &'a ChainSpec,
    pub slot_clock: &'a T::SlotClock,
    pub config: &'a ChainConfig,
    pub block_times_cache: &'a Arc<RwLock<BlockTimesCache>>,
...
pub fn filter_chain_segment<T: BeaconChainTypes>(
    ...
    for (i, block) in chain_segment.into_iter().enumerate() {
        ...
        if let Err(e) = block.as_block().fork_name(&chain.spec) {
...
pub async fn process_chain_segment<T: BeaconChainTypes>(
    ...
    for block in chain_segment.iter() {
        if let Err(error) = check_invalid_block_roots(chain, block.block_root()) {
...
pub async fn process_gossip_blob<T: BeaconChainTypes>(
    ...
    if chain.spec.is_peer_das_enabled_for_epoch(blob.epoch()) {
        return Err(BlockError::BlobNotRequired(blob.slot()));
    }

    let ctx = block_import_context_from_chain(chain);
    emit_sse_blob_sidecar_events(&ctx, &block_root, std::iter::once(blob.as_blob()));

    check_gossip_blob_availability_and_import(chain, blob).await
}
...
pub async fn process_rpc_blobs<T: BeaconChainTypes>(
    ...
    check_rpc_blob_availability_and_import(chain, slot, block_root, blobs).await
}
...
pub async fn process_engine_blobs<T: BeaconChainTypes>(
    ...
    check_engine_blobs_availability_and_import(chain, slot, block_root, engine_get_blobs_output)
        .await
}
...
pub async fn process_rpc_custody_columns<T: BeaconChainTypes>(
    ...
    check_rpc_custody_columns_availability_and_import(chain, slot, block_root, custody_columns)
        .await
}
...
> {
    ...
    let data_availability_checker = chain.data_availability_checker.clone();
    ...
    match result {
        DataColumnReconstructionResult::Success((availability, data_columns_to_publish)) => {
            ...
            process_availability(chain, slot, availability, || Ok(()))
                .await
...
pub async fn process_block<T: BeaconChainTypes, B: IntoExecutionPendingBlock<T>>(
    ...
    if !unverified_block
        ...
        .gloas_enabled()
    {
        chain.data_availability_checker.put_pre_execution_block(
            block_root,
    ...
    let import_block = async move {
        ...
        let executed_block = into_executed_block(chain_clone, execution_pending)
            .await
            .inspect_err(|_| {
        ...
        match executed_block {
            ExecutedBlock::Available(block) => import_available_block(chain, Box::new(block)).await,
            ExecutedBlock::AvailabilityPending(block) => {
                check_block_availability_and_import(chain, block).await
            }
        }
    };
...
async fn check_block_availability_and_import<T: BeaconChainTypes>(
    ...
    let availability = chain.data_availability_checker.put_executed_block(block)?;
    process_availability(chain, slot, availability, || Ok(())).await
}
...
async fn check_gossip_data_columns_availability_and_import<T: BeaconChainTypes>(
    ...
    process_availability(chain, slot, availability, publish_fn).await
}
...
async fn check_rpc_custody_columns_availability_and_import<T: BeaconChainTypes>(
    ...
    let availability = chain.data_availability_checker.put_rpc_custody_columns(
        block_root,
        slot,
        custody_columns,
    )?;
...
async fn process_availability<T: BeaconChainTypes>(
    ...
    match availability {
        Availability::Available(block) => {
            ...
            import_available_block(chain, block).await
        }
...
pub async fn import_available_block<T: BeaconChainTypes>(
    ...
    if let Some(blobs_available) = block.blobs_available_timestamp() {
        chain.block_times_cache.write().set_time_blob_observed(
            block_root,
            block.slot(),
            blobs_available,
        );
...
fn import_block<T: BeaconChainTypes>(
    ...
    check_block_against_weak_subjectivity_checkpoint(chain, block, block_root, &state)?;
    ...
    let old_head_slot = chain.canonical_head.cached_head().head_slot();
    ...
    let fork_choice_reader = chain.canonical_head.fork_choice_upgradable_read_lock();
        ...
        && block.slot() + EARLY_ATTESTER_CACHE_HISTORIC_SLOTS >= current_slot
    {
        ...
        match fork_choice.get_head(current_slot, &chain.spec) {
            ...
            Ok((new_head_root, _)) if new_head_root == block_root => {
                if let Some(proto_block) = fork_choice.get_block(&block_root) {
                        ...
                        .add_head_block(block_root, &signed_block, proto_block, &state)
                    {
                        ...
                        chain.block_times_cache.write().set_time_attestable(
                            block_root,
                            signed_block.slot(),
    ...
    import_block_observe_attestations(&ctx, block, &state, &mut consensus_context, current_epoch);
    ...
    import_block_update_slasher(&ctx, block, &state, &mut consensus_context);
    ...
    if let Err(e) = chain.store.do_atomically_with_block_and_blobs_cache(ops) {
        error!(
            msg = "Restoring fork choice from disk",
            error = ?e,
            "Database write failed!"
        );
        return Err(handle_import_block_db_write_error(&ctx, fork_choice)
            .err()
            .unwrap_or(e.into()));
    }
    ...
    // about it.
    let block_time_imported = chain.slot_clock.now_duration().unwrap_or(Duration::MAX);
...
pub fn check_block_against_weak_subjectivity_checkpoint<T: BeaconChainTypes>(
    ...
    let Some(wss_checkpoint) = chain.config.weak_subjectivity_checkpoint else {
        return Ok(());
    };
...
fn import_block_update_validator_monitor<T: BeaconChainTypes>(
    ...
    ctx.validator_monitor.write().process_valid_state(
        current_slot.epoch(T::EthSpec::slots_per_epoch()),
        state,
        ctx.spec,
    );

    let validator_monitor = ctx.validator_monitor.read();
...
fn import_block_observe_attestations<T: BeaconChainTypes>(
    ...
    for a in block.body().attestations() {
        ...
        let mut observed_block_attesters = ctx.attestation_manager.observed_block_attesters.write();
...
fn import_block_update_metrics_and_events<T: BeaconChainTypes>(
    ...
    let block_delay_total = get_slot_delay_ms(block_time_imported, block.slot(), ctx.slot_clock);

    // Do not write to the cache for blocks older than 2 epochs, this helps reduce writes to
    // the cache during sync.
    if block_delay_total < ctx.slot_clock.slot_duration() * 64 {
        // Store the timestamp of the block being imported into the cache.
        ctx.block_times_cache.write().set_time_imported(
            block_root,
            current_slot,
            block_time_imported,
        );
    ...
    // during sync.
    if ctx.config.enable_light_client_server
        && block_delay_total < ctx.slot_clock.slot_duration() * 32
        && let Some(mut light_client_server_tx) = ctx.light_client_server_tx.clone()
...
fn check_blob_header_signature_and_slashability<'a, T: BeaconChainTypes>(
    ...
    let mut slashable_cache = ctx.observed_slashable.write();
    for header in blobs
        .into_iter()
        ...
        .unique()
    {
        ...
        verify_header_signature::<T, BlockError>(chain, &header)?;
...
fn handle_import_block_db_write_error<T: BeaconChainTypes>(
    ...
    ctx.attestation_manager.early_attester_cache.clear();
    ...
    // fork choice.
    if let Err(e) = ctx.canonical_head.restore_from_store(
        fork_choice_write_lock,
        ResetPayloadStatuses::always_reset_conditionally(ctx.config.always_reset_payload_statuses),
        ctx.store,
        ctx.spec,
    ) {
        crit!(
