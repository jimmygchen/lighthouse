...
pub async fn produce_block_with_verification_gloas<T: BeaconChainTypes>(
    ...
    let (state, state_root_opt) = chain
        ...
        .spawn_blocking_handle(
            move || super::load_state_for_block_production(&chain_clone, slot),
...
pub async fn produce_block_on_state_gloas<T: BeaconChainTypes>(
    ...
    let graffiti = chain
        ...
        .await;
    let (partial_beacon_block, state) = chain
        .task_executor
        .spawn_blocking_handle(
            move || {
    ...
    let (execution_payload_bid, state, payload_data) = produce_execution_payload_bid(
        chain.clone(),
        state,
        produce_at_slot,
...
fn complete_partial_beacon_block_gloas<T: BeaconChainTypes>(
    ...
    if block_size > chain.config.max_network_size {
...
> {
    ...
    let proposer_index = state.get_beacon_proposer_index(state.slot(), &chain.spec)? as u64;
