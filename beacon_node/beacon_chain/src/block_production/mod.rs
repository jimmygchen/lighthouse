//! Block production: state loading, partial block assembly, execution payload integration,
...
use bls::Signature;
use execution_layer::{
    BlockProposalContents, BlockProposalContentsType, BuilderParams, ExecutionLayer,
};
...
pub struct BlockProducer<T: BeaconChainTypes> {
    ...
    pub(crate) op_pool: Arc<OperationPool<T::EthSpec>>,
...
impl<T: BeaconChainTypes> BlockProducer<T> {
    /// Create a new `BlockProducer` from its injected dependencies.
    ...
    fn get_state_for_re_org(
        ...
        let head_late =
            self.block_observed_after_attestation_deadline(canonical_head_root, head_slot);
        if !head_late {
    ...
    pub async fn produce_block_with_verification(
        ...
        let (state, state_root_opt) = self
            ...
            .spawn_blocking_handle(
                move || self_clone.load_state_for_block_production(slot),
    ...
    pub async fn produce_block_on_state(
        ...
        let mut partial_beacon_block = self
            ...
            .spawn_blocking_handle(
                move || {
                    self_clone.produce_partial_beacon_block(
                        state,
                        state_root_opt,
                        produce_at_slot,
                        randao_reveal,
            ...
            match block_contents_type {
                BlockProposalContentsType::Full(block_contents) => {
                    ...
                    let beacon_block_response = self
                        ...
                        .spawn_blocking_handle(
                            move || {
                                self_clone.complete_partial_beacon_block(
    ...
    pub(crate) fn get_pre_payload_attributes(
        &self,
        proposal_slot: Slot,
    ...
    pub(crate) fn get_expected_withdrawals_for_proposal(
        &self,
    ...
    #[allow(clippy::too_many_arguments)]
    #[instrument(skip_all, level = "debug")]
    pub(crate) fn produce_partial_beacon_block(
        self: &Arc<Self>,
    ...
    #[instrument(skip_all, level = "debug")]
    pub(crate) fn complete_partial_beacon_block<Payload: AbstractExecPayload<T::EthSpec>>(
        &self,
