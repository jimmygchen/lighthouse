...
//! 3. `Mutex<()>`: Is used to prevent concurrent execution of `BeaconSystem::recompute_head`.
//!
...
use crate::{
    BeaconChainError as Error, BeaconChainTypes, BeaconSnapshot, BeaconSystem,
    beacon_components::{
...
/// Provides a series of cached values from the last time `BeaconSystem::recompute_head` was run.
///
...
impl<E: EthSpec> CachedHead<E> {
    ...
    /// ## Notes
    ///
    /// This is *not* the current slot as per the system clock. Use `BeaconSystem::slot` for the
...
pub struct CanonicalHead<T: BeaconChainTypes> {
    ...
    /// A lock used to prevent concurrent runs of `BeaconSystem::recompute_head`.
    ///
...
impl<T: BeaconChainTypes> CanonicalHead<T> {
    ...
    /// run `BeaconSystem::recompute_head` to update the cached values.
...
pub async fn recompute_head_at_current_slot<T: BeaconChainTypes>(chain: &Arc<BeaconSystem<T>>) {
...
/// such a case it's critical that the `BeaconSystem` keeps importing blocks so that the
...
fn after_finalization<T: BeaconChainTypes>(
            ...
            .gloas_enabled()
    {
        ...
        // Use the `StateRootsIterator` directly rather than `BeaconSystem::state_root_at_slot`
