...
pub fn import_historical_data_column_batch<T: BeaconChainTypes>(
    ...
    for block_iter_result in forward_blocks_iter {
        ...
        for column_index in unique_column_indices.clone() {
                ...
                slot_and_column_index_to_data_columns.remove(&(slot, column_index))
            {
                ...
                chain
                    .store
                    .data_column_as_kv_store_ops(&block_root, data_column.clone(), &mut ops);
                total_imported += 1;
            }
        }
    }
    ...
    if historical_data_column_sidecar_list.is_empty() {
        ...
        verify_kzg_for_data_column_list(historical_data_column_sidecar_list.iter(), &chain.kzg)
            .map_err(|_| HistoricalDataColumnError::InvalidKzg)?;

        chain.store.blobs_db.do_atomically(ops)?;
    }
