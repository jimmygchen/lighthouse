...
pub fn check_database_invariants<T: BeaconChainTypes>(
    ...
    let ctx = InvariantContext {
        fork_choice_blocks,
        state_cache_roots: chain.store.state_cache.lock().state_roots(),
        custody_columns: custody_context
            .custody_columns_for_epoch(None, &chain.spec)
            .to_vec(),
        pubkey_cache_pubkeys: {
            let cache = chain.validator_query.validator_pubkey_cache.read();
            (0..cache.len())
                .filter_map(|i| {
                    cache.get(i).map(|pk| {
                        use store::StoreItem;
    ...
    chain.store.check_invariants(&ctx)
}
