...
/// Verify that a direct execution payload bid is consistent with the current chain state and
/// proposer preferences. Gossip-only payment checks are applied by the gossip verifier.
