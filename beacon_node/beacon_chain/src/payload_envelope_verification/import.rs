...
pub async fn process_execution_payload_envelope<T: BeaconChainTypes>(
    ...
    let chain_outer = chain.clone();
    let import_envelope = async move {
        let chain = chain_clone;
        let execution_pending =
            unverified_envelope.into_execution_pending_envelope(&chain, notify_execution_layer)?;
        publish_fn()?;
        ...
        let executed_envelope = into_executed_payload_envelope(chain.clone(), execution_pending)
            .await
            .inspect_err(|_| {
        ...
        match executed_envelope {
            ExecutedEnvelope::Available(envelope) => {
                import_available_execution_payload_envelope(&chain, Box::new(envelope)).await
            }
...
fn import_execution_payload_envelope<T: BeaconChainTypes>(
    ...
    // about it.
    let envelope_time_imported = chain.slot_clock.now_duration().unwrap_or(Duration::MAX);
    ...
    import_envelope_update_metrics_and_events(
        chain,
        signed_envelope,
        block_root,
        payload_verification_status,
...
fn import_envelope_update_metrics_and_events<T: BeaconChainTypes>(
    ...
    let envelope_delay_total =
        get_slot_delay_ms(envelope_time_imported, envelope_slot, &chain.slot_clock);
