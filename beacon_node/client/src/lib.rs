...
impl<T: BeaconChainTypes> Client<T> {
    /// Returns an `Arc` reference to the client's `BeaconSystem`, if it was started.
    pub fn beacon_chain(&self) -> Option<Arc<BeaconSystem<T>>> {
