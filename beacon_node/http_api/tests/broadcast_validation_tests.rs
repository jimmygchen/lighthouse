...
pub async fn consensus_partial_pass_only_consensus() {
    ...
    let gossip_block_b = block_b.into_gossip_verified_block(&tester.harness.chain.block_importer);
    assert!(gossip_block_b.is_ok());
    let gossip_block_a = block_a.into_gossip_verified_block(&tester.harness.chain.block_importer);
...
pub async fn blinded_equivocation_consensus_late_equivocation() {
    ...
    if !tester.harness.spec.is_fulu_scheduled() {
        ...
        let gossip_block_b =
            GossipVerifiedBlock::new(inner_block_b, &tester.harness.chain.block_importer);
        assert!(gossip_block_b.is_ok());
        let gossip_block_a =
            GossipVerifiedBlock::new(inner_block_a, &tester.harness.chain.block_importer);
        assert!(gossip_block_a.is_err());
...
pub async fn block_seen_on_gossip_without_blobs_or_columns() {
    ...
    block
        .clone()
        .into_gossip_verified_block(&tester.harness.chain.block_importer)
        .unwrap();
