use beacon_chain::{BeaconChainTypes, BeaconSystem, enr_fork_id};
...
///
/// NOTE: The purpose of this is simply to obtain a `StatusMessage` from the `BeaconSystem` without
/// polluting/coupling the type with RPC concepts.
pub trait ToStatusMessage {
    fn status_message(&self) -> StatusMessage;
}

impl<T: BeaconChainTypes> ToStatusMessage for BeaconSystem<T> {
    fn status_message(&self) -> StatusMessage {
        status_message(self)
    }
}
...
pub(crate) fn status_message<T: BeaconChainTypes>(beacon_chain: &BeaconSystem<T>) -> StatusMessage {
