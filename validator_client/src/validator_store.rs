...
impl<T: SlotClock + 'static, E: EthSpec> ValidatorStore<T, E> {
    ...
    /// builder payload.
    pub fn get_prefer_builder_proposals(&self, validator_pubkey: &PublicKeyBytes) -> Option<bool> {
        self.validators
            .read()
    ...
    pub fn determine_validator_builder_boost_factor(
        ...
        let validator_prefer_builder_proposals =
            self.get_prefer_builder_proposals(validator_pubkey);

        if matches!(validator_prefer_builder_proposals, Some(true)) {
            return Some(u64::MAX);
        }

        self.get_builder_boost_factor(validator_pubkey).or_else(|| {
            if matches!(
                self.validators.read().builder_proposals(validator_pubkey),
                Some(false)
            ) {
